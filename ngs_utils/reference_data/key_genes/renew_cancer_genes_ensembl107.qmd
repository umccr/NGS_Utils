...
```{r}
mart <- biomaRt::useEnsembl(biomart = "genes", dataset = "hsapiens_gene_ensembl", version = ensembl_version)
# can also grab "ensembl_gene_id"
attributes <- c("hgnc_symbol", "chromosome_name", "start_position", "end_position", "ensembl_gene_id")
filters <- "hgnc_symbol"
values <- list("hgnc_symbol" = umccr_genes)
# Total: 1,186
d <- biomaRt::getBM(attributes=attributes, filters=filters, values=values, mart=mart) |>
  as_tibble() |>
